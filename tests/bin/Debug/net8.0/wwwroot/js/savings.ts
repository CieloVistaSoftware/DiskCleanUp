...
let _savingsPage:    any[]  = [];
let _savingsOffset:  number = 0;
let _savingsTotal:   number = 0;
let _savingsSession: any    = null;
...
export async function updateTotalSaved() {
  try {
    ...
    ErrLog.log('[SAVINGS]', (ex as Error).message, (ex as Error).stack, 'CAUGHT_ERROR');
  }
}
...
async function _fetchSavingsPage(isFirst: boolean) {
  try {
    const resp = await apiFetch(
      `/api/savings?limit=${SAVINGS_PAGE_SIZE}&offset=${_savingsOffset}`,
      {}, { timeout: 20000 }
    );
    ...
    const entries: any[] = resp.entries || [];
    ...
    if (isFirst) {
      ...
      if (el) el.innerHTML = '<p class="empty-msg">⚠️ Could not load savings — backend offline?</p>';
    }
  }
}

function _renderSavings(data: any[], session: any, hasMore: boolean) {
  const total          = data.reduce((a: number, b: any) => a + (b.bytes || 0), 0);
  const sessionEntries = data.filter((e: any) => e.session_id === session?.id);
  const sessionTotal   = sessionEntries.reduce((a: number, b: any) => a + (b.bytes || 0), 0);
  ...
  if (!data.length) {
    const el = document.getElementById('savingsLog');
    if (el) el.innerHTML = '<p class="empty-msg">No savings recorded yet. Run a scan and delete some files!</p>';
    return;
  }
  ...
  groups.forEach((entries, sid) => {
    ...
    const groupTotal = entries.reduce((a: number, b: any) => a + (b.bytes || 0), 0);
...
export function filterSavings() {
  ...
  const action = (document.getElementById('savingsActionFilter') as HTMLSelectElement | null)?.value || '';
...
export function savingsSelectNone() {
  document.querySelectorAll('#savingsLog input[type=checkbox]')
...
export async function restoreSavingsSelected() {
  ...
  try {
    ...
    const matches  = binItems.filter((item: any) =>
    ...
    const recyclePaths = matches.map((m: any) => m.recyclePath);
