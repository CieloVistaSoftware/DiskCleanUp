...
export function create(section, containerId, columns, opts = {}) {
  ...
  const actionsCol = { key: '_actions', label: 'Actions', width: 120, type: 'actions' };
  ...
  const gridTemplate = cols.map(c => {
    if (typeof c._width === 'number') return `${c._width}px`;
    if (typeof c._width === 'string' && c._width.endsWith('px')) return c._width;
    return c.flex ? `minmax(${c.minWidth || 80}px, ${c.flex}fr)` : `${c._width}`;
  }).join(' ');
  ...
  g.columns.forEach(col => {
    ...
    switch (col.type) {
      ...
      case 'path': {
        ...
        const extDot = dot(ext);
        cell.innerHTML = `${extDot}<span class="sg-path-text" style="color:${colorFor(ext) || 'inherit'}">${_esc(data[col.key] || '')}</span>`;
      ...
      case 'actions': {
        ...
        folderBtn.title = 'Open containing folder';
...
export function applyFilter(section) {
  ...
  for (const row of g.rows) {
    ...
    if (text && !path.toLowerCase().includes(text)) vis = false;
...
function _rebuildLegend(section, g) {
  ...
  if (g._legendLastCount === sorted.length) return;
  ...
  g.legend.innerHTML = `<span class="sg-legend-label">File types (${sorted.length}):</span>` +
    sorted.map(ext =>
      `<span class="sg-legend-chip" title="${ext}">${dot(ext)}<span style="color:${colorFor(ext)}">${ext}</span></span>`
    ).join('');
}
...
function _saveWidths(section, widths) {
  try { localStorage.setItem(`sg_widths_${section}`, JSON.stringify(widths)); } catch {}
}
