...
export async function initKeepList() {
  ...
  if (btn) btn.addEventListener('click', _openKeepListModal);
}
...
export async function keepSelected(tableId) {
  ...
  if (window._scanGrid) {
    ...
    if (sec) paths = window._scanGrid.getChecked(sec);
  }
  if (!paths.length) {
...
export async function clearKeepList() {
  if (!confirm('Remove all kept files? They will appear in scans again.')) return;
  try {
...
function _updateBadge() {
  ...
  if (badge) {
    ...
    badge.style.display = _keepCount > 0 ? '' : 'none';
  }
}
...
async function _keepListClearAll() {
  ...
  // Refresh the modal
  const body = document.getElementById('keepListBody');
  if (body) body.innerHTML = '<p class="kl-empty">Keep list cleared.</p>';
}
