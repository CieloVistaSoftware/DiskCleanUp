...
export function apply(section) {
  ...
  for (const row of body.querySelectorAll('.sg-row:not(.sg-skel-row)')) {
    ...
    if (text && !path.toLowerCase().includes(text)) visible = false;

    if (visible && f.mode === 'include') {
      const sel = _getDropdownValue(section);
      if (sel && ext !== sel) visible = false;
  ...
  if (f.onApply) f.onApply(section, shown, total);
}
...
function _clearSaved(section) {
  try { localStorage.removeItem(_LS_PREFIX + section); } catch { /* ignore */ }
}
...
function _injectUI(section) {
  ...
  container.innerHTML = `
    <div class="sf-row">
      ...
      <input  id="sf-text-${section}" type="text" class="filter-input sf-text" placeholder="Filter by path\u2026" style="width:180px">
...
function _rebuildChips(section) {
  ...
  container.innerHTML = sorted.map(ext =>
    `<span class="sf-chip">${ext}<button class="sf-chip-x" data-ext="${ext}" title="Remove">&times;</button></span>`
  ).join('');
...
function _updateStatus(section, shown, total) {
  ...
  if (shown === total) { el.textContent = `${total.toLocaleString()} rows`; return; }
