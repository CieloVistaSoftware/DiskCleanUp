...
const ColumnTypes = {
  ...
  paths: {
    ...
    render: (items, data) => {
      ...
      return `<div class="sg-cell-paths">${items.map(item => 
        `<div class="sg-subitem">${escapeHtml(item)}</div>`
      ).join('')}</div>`;
    }
  }
};
...
export const GridShell = {
  ...
  showSkeleton(id, rows = 5) {
    const grid = grids.get(id);
    if (!grid || grid.skeletonShown) return;
  ...
  enableResize(id) {
    ...
    const headerCells = grid.headerEl.children;
  ...
  _createRowElement(grid, data) {
    ...
    if (grid.options.onRowClick || grid.options.onRowDblClick) {
      rowEl.addEventListener('click', () => {
        if (grid.options.onRowClick) grid.options.onRowClick(data, rowEl);
      });

      rowEl.addEventListener('dblclick', () => {
        if (grid.options.onRowDblClick) grid.options.onRowDblClick(data, rowEl);
      });
    }
