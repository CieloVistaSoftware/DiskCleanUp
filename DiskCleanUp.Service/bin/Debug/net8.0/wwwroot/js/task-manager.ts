...
function classify(proc) {
  ...
  if (proc.pid <= 4) return 'critical';  // System + System Idle

  // 2. Known safe user apps — always safe
  if (SAFE_APPS.has(name)) return 'safe';

  // 3. svchost is always caution (hosts OS services)
  if (name === 'svchost') return 'caution';

  // 4. Exact caution matches
  if (CAUTION_EXACT.has(name)) return 'caution';

  // 5. Pattern-based caution (drivers, GPU, peripherals)
  for (const pat of CAUTION_PATTERNS) {
    if (name.includes(pat)) return 'caution';
  }
  ...
  for (const co of CAUTION_COMPANIES) {
    if (company.includes(co)) return 'caution';
  }

  // 8. No path at all (system-level, can't inspect) → caution
  if (!path) return 'caution';
...
function _detectWindowApps(procs) {
  ...
  for (const p of procs) {
    ...
    if (!_windowApps.has(name)) _windowApps.set(name, new Set());
...
function _getParentAppTitle(proc) {
  ...
  if (title) return null; // has its own title — not a hidden child
  const name = (proc.name || '').toLowerCase();
...
function render() {
  ...
  _displayed = _allProcs.filter(p => {
    // Safety filter
    if (safetyFilter && p.tier !== safetyFilter) return false;
    // Text filter
    if (textFilter) {
...
async function killSelected() {
  ...
  for (const pid of pids) {
    try {
      ...
      else errors.push(d.error || `PID ${pid} failed`);
    } catch (e) {
      ErrLog.log('[TASK_MANAGER]', e.message, e.stack, 'CAUGHT_ERROR');
