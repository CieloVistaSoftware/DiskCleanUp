...
// â”€â”€ CSS Merge Viewer helpers â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

function _cssKb(chars) {
  return chars >= 1024 ? `${(chars / 1024).toFixed(1)} KB` : `${chars} B`;
}
function _cssRiskColor(r) {
  return r === "high" ? "#e74c3c" : r === "medium" ? "#f39c12" : "#2ecc71";
}
function _cssRiskBadge(r) {
  return `<span style="background:${_cssRiskColor(r)};color:#000;font-size:10px;font-weight:700;padding:1px 6px;border-radius:3px">${r.toUpperCase()}</span>`;
}

function _buildGroupCard(g, i) {
  ...
  const filesHtml = (g.files ?? []).map((f) => {
    ...
    return `<span style="background:var(--surface2,#2a2a2a);padding:2px 8px;border-radius:3px;display:inline-flex;align-items:center;gap:4px">
      ...
      <span style="color:var(--muted,#888)">${_cssKb(f.sizeChars || 0)} \xB7 ${f.selectorCount} sel</span>
    </span>`;
  }).join('<span style="color:var(--muted,#666);padding:0 2px">+</span>');
  const conflictsHtml = g.conflictCount > 0
    ? `<details style="margin-top:5px"><summary style="cursor:pointer;color:#f39c12;font-size:11px">âš  ${g.conflictCount} selector conflict${g.conflictCount !== 1 ? "s" : ""} â€” click to expand</summary>
    ...
    : `<span style="font-size:11px;color:#2ecc71">âœ“ No conflicts</span>`;
  return `<div class="css-merge-card" data-idx="${i}" style="border:1px solid var(--border,#333);border-radius:6px;margin-bottom:8px;overflow:hidden">
    <div style="display:flex;align-items:flex-start;gap:8px;padding:10px 12px;background:var(--surface,#1e1e1e)">
      ...
      <div style="flex:1;min-width:0">
        <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin-bottom:6px">
          ...
          ${_cssRiskBadge(g.risk)}
          <span style="font-size:11px;color:var(--muted,#888)">${_cssKb(totalBytes)} total</span>
        </div>
        <div style="display:flex;align-items:flex-start;gap:6px;flex-wrap:wrap;margin-bottom:6px;font-size:11px">
          <div style="display:flex;flex-wrap:wrap;gap:4px;align-items:center">${filesHtml}</div>
          <span style="color:#3fb950;font-size:13px;font-weight:700;align-self:center">â†’</span>
      ...
      <div style="display:flex;flex-direction:column;gap:5px;flex-shrink:0">
        ...
        <button class="css-dry-run-btn" data-idx="${i}" style="background:#2a2a1a;color:#e3b341;border:1px solid #e3b341;padding:4px 12px;border-radius:4px;font-size:11px;cursor:pointer;white-space:nowrap" title="Write merged.PREVIEW.css â€” originals untouched.">\u{1F9EA} Dry Run</button>
        <button class="css-merge-one-btn" data-idx="${i}" style="background:#1a3a1a;color:#3fb950;border:1px solid #3fb950;padding:4px 12px;border-radius:4px;font-size:11px;cursor:pointer;white-space:nowrap" title="Merge just this group â€” originals saved as .bak">â–¶ Merge This</button>
      </div>
    </div>
    <div class="css-preview-panel" data-idx="${i}" style="display:none;border-top:1px solid var(--border,#333)">
      <div style="padding:6px 12px;background:#0d1117;display:flex;align-items:center;justify-content:space-between">
        ...
        <button class="css-preview-close" data-idx="${i}" style="background:none;border:none;color:var(--muted,#888);cursor:pointer;font-size:13px">âœ• Close</button>
      </div>
      <div class="css-preview-content" data-idx="${i}" style="margin:0;font:12px/1.6 'Cascadia Code','Consolas',monospace;background:#0d1117;overflow:auto;max-height:340px">
        <div style="padding:6px 14px;color:#8b949e;font-style:italic">Loading previewâ€¦</div>
      </div>
    </div>
  </div>`;
}

function _buildRefsHtml(refs) {
  if (!refs.length) return '<p style="color:var(--muted,#666);font-size:12px">No referencing HTML/JS files found in the same or parent folder (search limited to 1 level up to avoid cross-project noise).</p>';
  return refs.map((r) => `
    <div style="margin-bottom:8px;font-size:12px;border-left:2px solid var(--border,#333);padding-left:8px">
      ...
      ${!(r.referencedBy ?? []).length
        ? '<div style="color:var(--muted,#666);font-size:11px">No references found nearby</div>'
        : r.referencedBy.map((f) => `<div style="color:var(--muted,#888);font-size:11px">â†’ ${f}</div>`).join("")}
    </div>`).join("");
}

function _buildMergeOverlayHtml(res, groups, groupCards, refsHtml, errHtml) {
  const refs = res?.refs ?? [];
  const errors = res?.errorFiles ?? [];
  const bodyContent = groups.length === 0
    ? '<p style="color:var(--muted,#666)">No merge candidates found â€” each scanned folder contains only one CSS file.</p>'
    : `<div style="display:flex;align-items:center;gap:8px;margin-bottom:10px">
         ...
         <span style="font-size:11px;color:var(--muted,#666)">â€” uncheck groups you want to skip</span>
       </div>
       ${groupCards.join("")}`;
  const refSection = refs.length > 0 || errors.length > 0
    ? `<h4 style="margin:14px 0 6px;font-size:12px;text-transform:uppercase;color:var(--muted,#888);letter-spacing:.05em">CSS References in HTML/JS</h4>${refsHtml}${errHtml}`
    : "";
  return `
    <div style="background:var(--bg,#121212);border:1px solid var(--border,#333);border-radius:8px;width:min(960px,96vw);max-height:90vh;display:flex;flex-direction:column;overflow:hidden;box-shadow:0 24px 80px rgba(0,0,0,.9)">
      <div style="padding:11px 16px;border-bottom:1px solid var(--border,#333);display:flex;justify-content:space-between;align-items:center;flex-shrink:0">
        <strong style="font-size:14px">\u{1F52C} CSS Merge Viewer â€” ${res?.analyzedCount ?? 0} files analyzed \xB7 <span id="css-mv-gcount">${groups.length}</span> group${groups.length !== 1 ? "s" : ""}${res?.scopeApplied ? ` \xB7 scope: ${res.scopeApplied}` : ""}</strong>
        <div style="display:flex;gap:8px;align-items:center">
          ...
          <button id="css-merge-close" style="background:none;border:none;font-size:20px;cursor:pointer;color:var(--fg,#eee);line-height:1">âœ•</button>
        </div>
      </div>
      <div style="overflow-y:auto;padding:12px 14px;flex:1">${bodyContent}${refSection}</div>
      <div style="padding:10px 16px;border-top:1px solid var(--border,#333);display:flex;align-items:center;gap:10px;flex-shrink:0">
        ${groups.length > 0 ? `<button id="css-merge-exec-btn" style="background:#2a5c2a;color:#3fb950;border:1px solid #3fb950;padding:7px 22px;border-radius:5px;font-size:.85rem;font-weight:600;cursor:pointer" title="Merge all checked groups at once">â–¶ Merge All Checked</button>` : ""}
...
async function _wirePreviewBtn(overlay, groups, btn) {
  ...
  if (isOpen) { panel.style.display = "none"; btn.textContent = "\u{1F441} Preview"; return; }
  ...
  try {
    ...
    if (r === null) { pre.innerHTML = '<div style="padding:8px 14px;color:#e74c3c">Preview failed â€” check Errors panel.</div>'; return; }
    const conflictSelectors = new Set((groups[i].conflicts ?? []).map((c) => c.selector));
    const conflictMap = {};
    (groups[i].conflicts ?? []).forEach((c) => { conflictMap[c.selector] = c.files; });
    ...
    let html = `<div style="padding:8px 14px;background:#0a1a0a;border-bottom:1px solid #333;font-size:11px;color:#8b949e">
      Combining <strong style="color:#eee">${r.sections?.length ?? 0} files</strong> â†’
      <strong style="color:#3fb950">${(groups[i].mergedName || "").split(/[\\\/]/).pop()}</strong>
      &nbsp;\xB7&nbsp; ${_cssKb(r.sourceBytes)} source &nbsp;\xB7&nbsp; ~${totalSel} selectors total
      ${conflictCount > 0
        ? `&nbsp;\xB7&nbsp; <span style="color:#f39c12">âš  ${conflictCount} conflict${conflictCount !== 1 ? "s" : ""} highlighted â€” last definition wins</span>`
        : `&nbsp;\xB7&nbsp; <span style="color:#2ecc71">âœ“ No conflicts</span>`}
    </div>`;
    if (conflictCount > 0) {
    ...
    for (const sec of r.sections ?? []) {
      html += `<div style="border-top:2px solid #1f3a5f">
        <div style="padding:4px 14px;background:#0d1a2a;font-size:11px;color:#58a6ff;display:flex;gap:12px">
          ...
          <span style="color:#8b949e">${_cssKb(sec.sizeChars)}</span>
      ...
      for (const line of sec.lines) {
        ...
        let isConflict = false; let conflictFiles = null;
...
async function _wireDryRunBtn(overlay, groups, btn) {
  const i = Number(btn.dataset.idx);
  const group = groups[i];
  if (!group) return;
  btn.disabled = true;
  btn.textContent = "âŸ³ Writingâ€¦";
  try {
    const r = await apiFetch("/api/css/merge-dry-run", {
      method: "POST", headers: { "Content-Type": "application/json" },
      body: JSON.stringify(group),
    }, { timeout: 30000 });
    if (r === null) { btn.textContent = "âœ— Failed"; btn.disabled = false; return; }
    ...
    dryBar.innerHTML = `
      ...
      <button class="css-dryrun-apply" style="background:#1a3a1a;color:#3fb950;border:1px solid #3fb950;padding:3px 10px;border-radius:4px;font-size:11px;cursor:pointer;font-weight:600">â–¶ Apply â€” Replace Originals</button>
    ...
    dryBar.querySelector(".css-dryrun-apply").addEventListener("click", async () => {
      ...
      applyBtn.textContent = "âŸ³ Applyingâ€¦";
      await fetch("/api/css/merge-dry-run/discard", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ path: r.dryRunPath }) });
      try {
        ...
        if (mr === null) { applyBtn.textContent = "âœ— Failed"; applyBtn.disabled = false; return; }
        dryBar.remove();
        ...
        if (mergeOneBtn) { mergeOneBtn.textContent = "âœ… Merged"; mergeOneBtn.disabled = true; }
...
async function _wireMergeOneBtn(overlay, groups, btn) {
  const i = Number(btn.dataset.idx);
  const group = groups[i];
  if (!group) return;
  if (group.risk === "high" && !confirm(`HIGH RISK: ${group.conflictCount} selector conflicts in this group.\n\nThe last matching selector wins in CSS â€” some styles may change.\n\nOriginals are saved as .bak so you can undo.\n\nMerge anyway?`)) return;
  btn.disabled = true;
  btn.textContent = "âŸ³ Mergingâ€¦";
  const card = overlay.querySelector(`.css-merge-card[data-idx="${i}"]`);
  try {
...
function _wireMergeAllBtn(overlay, groups) {
  ...
  execBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      execBtn.textContent = "â–¶ Merge All Checked";
    }
  });
}
...
function _showCssMergeViewer(res) {
  ...
  const errHtml = errors.length === 0 ? "" : `
    <h4 style="margin:10px 0 4px;color:#e74c3c">âš  Parse Errors</h4>
    ${errors.map((e) => `<div style="font-size:11px;color:#e74c3c">${e.path || e.Path}: ${e.error || e.Error}</div>`).join("")}`;

  const groupCards = groups.map((g, i) => _buildGroupCard(g, i));
  const refsHtml = _buildRefsHtml(refs);

  const overlay = document.createElement("div");
  overlay.id = "css-merge-report-overlay";
  overlay.style.cssText = "position:fixed;inset:0;background:rgba(0,0,0,.7);z-index:9999;display:flex;align-items:center;justify-content:center";
  overlay.innerHTML = _buildMergeOverlayHtml(res, groups, groupCards, refsHtml, errHtml);
  document.body.appendChild(overlay);
  ...
  overlay.querySelectorAll(".css-preview-btn").forEach((btn) => {
    btn.addEventListener("click", () => _wirePreviewBtn(overlay, groups, btn));
  });
  ...
  overlay.querySelectorAll(".css-dry-run-btn").forEach((btn) => {
    btn.addEventListener("click", () => _wireDryRunBtn(overlay, groups, btn));
  });

  overlay.querySelectorAll(".css-merge-one-btn").forEach((btn) => {
    btn.addEventListener("click", () => _wireMergeOneBtn(overlay, groups, btn));
  });

  _wireMergeAllBtn(overlay, groups);
}
...
function renderImageGroups(resultEl, groups) {
  ...
  Object.values(groups).forEach((files) => {
    ...
    const groupKey = `img-group-${groupIdx++}`;
    html += `<div class="img-group" data-group="${groupKey}">
      <div class="img-cards-wrap">`;
    files.forEach((fp) => {
      const filename = (fp || "").split(/[\\\/]/).pop() || fp || "";
      const safePath = (fp || "").replace(/"/g, "&quot;").replace(/'/g, "&#39;");
      html += `<div class="img-card live-row" data-path="${safePath}">
        <div class="img-card-preview">
          <img src="/api/file?path=${encodeURIComponent(fp)}" alt="" onerror="this.classList.add('img-broken')">
        </div>
        <div class="img-card-body">
          <div class="img-card-filename">${filename}</div>
          <div class="img-card-path" title="${safePath}">${fp}</div>
        </div>
        <div class="img-card-foot">
          <button class="img-delete-btn" data-path="${safePath}" title="Send to Recycle Bin">\u{1F5D1} Delete</button>
        </div>
      </div>`;
    });
    html += "</div></div>";
  ...
  resultEl.querySelectorAll(".img-delete-btn").forEach((btn) => {
    btn.addEventListener("click", async () => {
      ...
      try {
        ...
        btn.disabled = false;
        btn.textContent = "\u{1F5D1} Delete";
        window._T?.("IMG", `Delete failed: ${e.message}`);
      }
    });
  });
  // Apply saved column preference via generic toolbar function
  window._colsApply?.("images");
}
async function trashImage(path) {
  // Legacy function for backwards compatibility — not used in new render
  // (trashImage is now wired via event listener in renderImageGroups)
  try {
    await fetch("/api/trash", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
async function trashAllImageCopies() {
  crumb("actions", "trashAllImageCopies");
  
  // Collect all image cards from the current UI (not from window._imageGroups)
  const resultEl = document.getElementById("imageResult");
  if (!resultEl) { alert("No images currently displayed."); return; }
  
  const cards = Array.from(resultEl.querySelectorAll(".img-card[data-path]"));
  const paths = cards.map(card => card.dataset.path || "").filter(p => p);
  
  if (!paths.length) { alert("No images found."); return; }
  if (!confirm(`Delete ALL ${paths.length} image${paths.length===1?'' : 's'}?\nFiles go to Recycle Bin.`)) return;
  ...
  try {
    ...
    setTimeout(() => {
      if (resultEl.children.length === 0) {
        resultEl.innerHTML = "<p class=\"empty-msg\">✅ All images deleted.</p>";
      }
    ...
    if (btn) { btn.disabled = false; btn.textContent = "🗑 Delete All"; }
    window._T?.("IMG", `Delete all failed: ${e.message}`);
