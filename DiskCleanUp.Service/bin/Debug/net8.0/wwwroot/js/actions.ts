...
// ── CSS Merge Viewer helpers ──────────────────────────────────────────────

function _cssKb(chars: number) {
  return chars >= 1024 ? `${(chars / 1024).toFixed(1)} KB` : `${chars} B`;
}
function _cssRiskColor(r: string) {
  return r === 'high' ? '#e74c3c' : r === 'medium' ? '#f39c12' : '#2ecc71';
}
function _cssRiskBadge(r: string) {
...
function _buildGroupCard(g: any, i: number): string {
  ...
  const filesHtml = (g.files ?? []).map((f: any) => {
    ...
    return `<span style="background:var(--surface2,#2a2a2a);padding:2px 8px;border-radius:3px;display:inline-flex;align-items:center;gap:4px">
      ...
      <span style="color:var(--muted,#888)">${_cssKb(f.sizeChars || 0)} · ${f.selectorCount} sel</span>
    </span>`;
  ...
  return `<div class="css-merge-card" data-idx="${i}" style="border:1px solid var(--border,#333);border-radius:6px;margin-bottom:8px;overflow:hidden">
    <div style="display:flex;align-items:flex-start;gap:8px;padding:10px 12px;background:var(--surface,#1e1e1e)">
      ...
      <div style="display:flex;flex-direction:column;gap:5px;flex-shrink:0">
        ...
        <button class="css-dry-run-btn" data-idx="${i}" style="background:#2a2a1a;color:#e3b341;border:1px solid #e3b341;padding:4px 12px;border-radius:4px;font-size:11px;cursor:pointer;white-space:nowrap" title="Write merged.PREVIEW.css — originals untouched.">🧪 Dry Run</button>
...
function _buildRefsHtml(refs: any[]): string {
...
function _buildMergeOverlayHtml(res: any, groups: any[], groupCards: string[], refsHtml: string, errHtml: string): string {
...
async function _wirePreviewBtn(overlay: HTMLElement, groups: any[], btn: Element): Promise<void> {
  ...
  try {
    ...
    let html = `<div style="padding:8px 14px;background:#0a1a0a;border-bottom:1px solid #333;font-size:11px;color:#8b949e">
      ...
      &nbsp;·&nbsp; ${_cssKb(r.sourceBytes)} source &nbsp;·&nbsp; ~${totalSel} selectors total
      ${conflictCount > 0
...
async function _wireDryRunBtn(overlay: HTMLElement, groups: any[], btn: Element): Promise<void> {
  const i = Number((btn as HTMLElement).dataset.idx);
...
async function _wireMergeOneBtn(overlay: HTMLElement, groups: any[], btn: Element): Promise<void> {
  const i = Number((btn as HTMLElement).dataset.idx);
  const group = groups[i];
  if (!group) return;
...
function _wireMergeAllBtn(overlay: HTMLElement, groups: any[]): void {
...
export function renderImageGroups(resultEl, groups) {
    ...
    files.forEach((fp) => {
      ...
      html += `<div class="img-card live-row" data-path="${safePath}">
        ...
        <div class="img-card-foot">
          <button class="img-delete-btn" data-path="${safePath}" title="Send to Recycle Bin">🗑 Delete</button>
        </div>
      </div>`;
    });
  ...
  resultEl.querySelectorAll('.img-delete-btn').forEach((btn: any) => {
    btn.addEventListener('click', async () => {
      const card = btn.closest('.img-card') as HTMLElement | null;
  ...
  (window as any)._colsApply?.('images');
}
...
export async function trashAllImageCopies() {
  crumb('actions', 'trashAllImageCopies');
  
  // Collect all image cards from the current UI (not from window._imageGroups)
  const resultEl = document.getElementById('imageResult') as HTMLElement | null;
  if (!resultEl) { alert('No images currently displayed.'); return; }
  
  const cards = Array.from(resultEl.querySelectorAll('.img-card[data-path]')) as HTMLElement[];
