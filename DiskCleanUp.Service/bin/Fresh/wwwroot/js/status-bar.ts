...
export const SB = {
  begin(section, root?) {
    ...
    } catch (err) { ErrLog.log('[status-bar]', String(err), null, 'STATUS_BAR_ERROR'); }
  },
...
function _addStopBtn(bar: HTMLElement, section: string) {
...
function _removeStopBtn(bar: HTMLElement) {
  const btn = bar.querySelector('.sb-stop-btn');
  if (btn) btn.remove();
}

export function _set(section: string, key: string, val: string) {
  try {
...
export function _folder(section: string, txt: string) {
  try {
...
export function _getVal(section: string, key: string) {
  try {
    const el = document.getElementById(`sbv-${section}-${key}`);
    return el ? el.textContent : '0';
  } catch (err) { ErrLog.log('[status-bar]', String(err), null, 'STATUS_BAR_ERROR'); return '0'; }
}
