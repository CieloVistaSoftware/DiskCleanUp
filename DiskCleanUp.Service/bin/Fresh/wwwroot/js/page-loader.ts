...
export async function loadPage(section) {
  ...
  try {
    ...
    if (data.cachedAt && !_cachedAt[section]) _cachedAt[section] = new Date(data.cachedAt);
    // Always track the actual byte position (even at EOF) so live-scan can resume
    if (data.resumeOffset != null) _resumeOffsets[section] = data.resumeOffset;
    ...
    ErrLog.log('[PAGE-LOADER]', e.message, e.stack, 'CAUGHT_ERROR');
    _offsets[section] = null;
    return { rows: [], loaded: _loaded[section] || 0 };
  }
}
