    ...
    try {
        ...
        if (config.src) {
            ...
            try {
                ...
                console.error('[wb-core:mdhtml] Load error:', err, config.src);
        ...
        try {
            element.setAttribute('x-hydrated', '1');
            element.dispatchEvent(new CustomEvent('wb:mdhtml:hydrated', { bubbles: true }));
        }
        catch (e) { /* best-effort */ }
        ...
        if (window.WB) {
            ...
            element.querySelectorAll('pre').forEach(el => {
                ...
                if (code) {
                    ...
                    if (langMatch) {
                        el.setAttribute('language', langMatch[1]);
                        code.setAttribute('language', langMatch[1]);
                    }
                    // Default options for markdown code blocks
                    el.setAttribute('show-line-numbers', 'true');
                    el.setAttribute('show-copy', 'true');
        ...
        element.classList.remove('wb-mdhtml--loading');
        element.classList.add('wb-mdhtml--error');
        console.error('[wb-core:mdhtml] Unexpected error:', err, config.src);
