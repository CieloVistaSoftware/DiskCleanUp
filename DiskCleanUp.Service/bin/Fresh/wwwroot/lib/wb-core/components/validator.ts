...
export function validator(element: HTMLElement, options: ValidatorOptions = {}) {
  const config = {
    validateOn: options.validateOn || element.getAttribute('validate-on') || 'blur', // blur, change, input, submit
    showErrors: options.showErrors ?? (element.getAttribute('show-errors') !== 'false'),
    errorClass: options.errorClass || element.getAttribute('error-class') || 'wb-error',
    successClass: options.successClass || element.getAttribute('success-class') || 'wb-success',
    ...options
  };
  ...
  const parseRules = (input) => {
    const ruleStr = input.getAttribute('validate') || '';
  ...
  const getInputs = () => {
    return Array.from(element.querySelectorAll('[validate]'));
  };
