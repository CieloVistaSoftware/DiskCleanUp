...
function openGalleryLightbox(images, index) {
  ...
  const render = () => {
    ...
    `;
    (overlay.querySelector('.wb-lightbox__prev') as HTMLElement).onclick = (e) => { e.stopPropagation(); current = (current - 1 + images.length) % images.length; render(); };
    (overlay.querySelector('.wb-lightbox__next') as HTMLElement).onclick = (e) => { e.stopPropagation(); current = (current + 1) % images.length; render(); };
    (overlay.querySelector('.wb-lightbox__close') as HTMLElement).onclick = () => overlay.remove();
  };
...
export function audio(element, options: Record<string, any> = {}) {
  const config = {
    src: options.src || element.getAttribute('src') || '',
    controls: options.controls ?? element.getAttribute('controls') !== 'false',
    autoplay: options.autoplay ?? (element.hasAttribute('autoplay') && element.getAttribute('autoplay') !== 'false'),
    loop: options.loop ?? (element.hasAttribute('loop') && element.getAttribute('loop') !== 'false'),
  ...
  setTimeout(() => {
    if (audioEl.error) {
      console.error('[audio]', new Error('Audio element error'), { context: 'load', code: audioEl.error.code });
    }
  }, 1000);
    ...
    if (enableEq) {
      ...
      audioEl.addEventListener('error', (e) => {
        ...
        if (audioEl.crossOrigin && audioEl.error && (audioEl.error.code === 4 || audioEl.error.code === 2)) {
          console.warn('[audio]', 'CORS/Network error, falling back to standard playback (EQ disabled)');
          ...
          audioEl.load();
          if (config.autoplay) audioEl.play().catch(e => console.warn('[audio]', 'Autoplay failed after CORS fallback', { stack: e?.stack }));
        }
      }, { once: true });
    }
  ...
  const initAudioContext = () => {
    ...
    if (!audioEl.crossOrigin) {
      console.warn('[audio]', 'Skipping EQ: No CORS access on audio element');
      return;
    }
    ...
    try {
      ...
      console.warn('[audio]', 'Web Audio API initialization failed', { stack: e?.stack });
    }
  };
  ...
  if (config.showEq) {
    ...
    masterVol.oninput = (e) => {
      audioEl.volume = (e.target as HTMLInputElement).valueAsNumber / 100;
    };
    ...
    EQ_BANDS.forEach((band, index) => {
      ...
      slider.oninput = (e) => {
        initAudioContext();
        const bandVal = parseFloat((e.target as HTMLInputElement).value);
        if (filters[index]) {
...
export function youtube(element, options: Record<string, any> = {}) {
  ...
  if (!config.id) {
    console.warn('[youtube]', 'No video-id attribute provided');
    return;
  }
...
export function vimeo(element, options: Record<string, any> = {}) {
  ...
  if (!config.id) {
    console.warn('[vimeo]', 'No video-id attribute provided');
    return;
  }
  ...
  videoIframe.src = `https://player.vimeo.com/video/${config.id}?${embedParams}`;
  videoIframe.allow = 'autoplay; fullscreen; picture-in-picture';
  videoIframe.allowFullscreen = true;
  videoIframe.style.cssText = 'width:100%;height:100%;border:none;';

  element.innerHTML = '';
  element.appendChild(videoIframe);
