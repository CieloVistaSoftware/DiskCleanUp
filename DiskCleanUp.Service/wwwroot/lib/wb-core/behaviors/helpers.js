...
export function lazy(element, options = {}) {
    ...
    try {
        observer = new IntersectionObserver((entries) => {
            try {
                entries.forEach(entry => {
                    if (entry.isIntersecting) {
                        ...
                        try {
                            ...
                            ErrLog.log('[HELPERS]', e.message, e.stack, 'CAUGHT_ERROR');
                        }
                    }
                });
            }
                ...
                ErrLog.log('[HELPERS]', cbErr.message, cbErr.stack, 'CAUGHT_ERROR');
            }
        ...
        ErrLog.log('[HELPERS]', instErr.message, instErr.stack, 'CAUGHT_ERROR');
    }
    ...
    try {
        ...
        console.debug('[wb-lazy] observer.observe failed', obsErr);
        ErrLog.log('[HELPERS]', obsErr.message, obsErr.stack, 'CAUGHT_ERROR');
    }
    return () => {
        try {
            observer.disconnect();
