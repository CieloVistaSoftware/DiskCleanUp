...
export function copy(element, options: Record<string, any> = {}) {
  ...
  const getTextToCopy = () => {
    ...
    if (config.target) {
      ...
      if (targetEl) return targetEl.value || targetEl.textContent;
    }
  ...
  const onClick = async (e) => {
    ...
    try {
      ...
      showFeedback();
      element.dispatchEvent(new CustomEvent('wb:copy:success', { bubbles: true, detail: { text } }));
    } catch (err) {
      ...
      try {
        ...
        element.dispatchEvent(new CustomEvent('wb:copy:error', { bubbles: true, detail: { error: e } }));
      }
