...
export class PubSub {
  ...
  _queue: Array<{ event: string; data: any }>;
  _rafPending: boolean;
  _batchSize: number;
  _memDropCount: number = 0;
  _drainCount: number = 0;
  _backoffLogged: boolean = false;
  ...
  publish(event, data) {
    ...
    subs.forEach(cb => {
      try { cb(data); }
      catch (err) { console.error(`[PubSub] Error in subscriber for "${event}":`, err); }
    });
  }

  publishBatched(event, data, opts: { dropWhenFull?: boolean; maxQueue?: number; lowPriority?: boolean } = {}) {
    const { dropWhenFull = false, maxQueue = 50, lowPriority = false } = opts;
    if (dropWhenFull && this._queue.length > maxQueue) return;
  ...
  _drain() {
    ...
    const mem = window._memPct || 0;
    let budgetMs = 8;
    if (cpu >= 70 || mem > 70) budgetMs = 4;
    if (cpu < 70 && mem <= 70) this._backoffLogged = false;
    ...
    if (this._queue.length > 0) {
      if (cpu >= 70 || mem > 70) setTimeout(() => this._drain(), 50);
      else requestAnimationFrame(() => this._drain());
    } else {
      this._rafPending = false;
