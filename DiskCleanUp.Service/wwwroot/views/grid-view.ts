      ...
      symlinkBtn.addEventListener('click', () => {
        ...
        fetch('/api/symlink', {
          ...
          if (!r.ok) {
            ...
            symlinkBtn.textContent = '🔗 Symlink';
            const msg = r.status === 404 || r.status === 405
              ? 'Symlink endpoint not found — restart the DiskCleanUp service to pick up the new endpoint (npm run restart).'
              : (j.detail || j.error || `Symlink failed (HTTP ${r.status})`);
            alert(msg);
            return;
          }
