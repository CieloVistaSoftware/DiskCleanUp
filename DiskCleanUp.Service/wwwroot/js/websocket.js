...
window._wsSend = (raw) => { if (_ws && _ws.readyState === WebSocket.OPEN)
    _ws.send(raw); };
...
async function _showFatalError() {
    ...
    // ── Step 1: probe the current port (relative URL) ──────────────────────
    let httpOk = false;
    let httpDetail = '';
    try {
    ...
    // ── Step 2: if current port is down, probe the alternate port ──────────
    //   :5100 (service/production) ↔ :5000 (console/dev)
    ...
    // ── Build buttons based on failure case ───────────────────────────────
...
export function wsConnect() {
    ...
    // If no onopen within 15s — diagnose and show fatal error panel
    clearTimeout(_connectTimer);
    let _oncloseWasCalled = false;
    ...
    _ws.onopen = () => {
        ...
        // If a fatal-error overlay is visible (from a previous failed attempt),
        // remove it now that we have a live connection.
    ...
    _ws.onmessage = (ev) => {
        ...
        if (msg.type === 'started') {
            ...
            // Clear any stale deferred done from a prior scan
            delete _deferredDone[msg.section];
        }
        // done/error: defer if _fetchBatch is still active for this section.
        // Otherwise vm.scanDone() would fire with vm.size === 0 (ROWS-ZERO-001).
