...
export function showSection(name, btn?) {
  ...
  if (btn) btn.classList.add('active');
    ...
    const navBtn = document.querySelector(`nav button[data-section="${name}"]`) as HTMLElement | null;
    if (navBtn) navBtn.classList.add('active');
  }

  // Keep section dropdown synced with the active section.
  const sectionMenu = document.getElementById('sectionMenu') as HTMLSelectElement | null;
  if (sectionMenu && sectionMenu.value !== name) sectionMenu.value = name;

  _activeSection = name;

  // Persist active tab so page refresh stays on this tab
  try { localStorage.setItem('dcu_active_tab', name); } catch (ex) {
