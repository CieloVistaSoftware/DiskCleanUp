...
function _showCssMergeReport(res: any) {
  ...
  const groupsHtml = groups.length === 0
    ...
    : groups.map((g: any) => `
      <div style="border:1px solid var(--border);border-radius:4px;padding:10px 14px;margin-bottom:10px">
        ...
        <div style="margin-bottom:4px">${(g.files ?? []).map((f: any) => `<span style="background:var(--surface2);padding:1px 6px;border-radius:3px;margin-right:4px;font-size:11px">${f.Path.split(/[\\/]/).pop()} (${f.selectorCount} selectors)</span>`).join('')}</div>
        ...
        ${g.conflictCount > 0 ? `<details style="margin-top:6px"><summary style="cursor:pointer;color:var(--muted);font-size:12px">Show conflicts</summary><ul style="margin:4px 0 0 16px;font-size:12px">${(g.conflicts ?? []).map((c: any) => `<li><code>${c.selector}</code> in: ${c.files.join(', ')}</li>`).join('')}</ul></details>` : ''}
    ...
    : refs.map((r: any) => `
      <div style="margin-bottom:8px">
        ...
        ${(r.referencedBy ?? []).length === 0
          ...
          : (r.referencedBy as string[]).map(f => `<div style="font-size:11px;color:var(--muted)">→ ${f}</div>`).join('')}
      </div>`).join('');

  const errHtml = errors.length === 0 ? '' : `
    <h3 style="margin:12px 0 6px">⚠ Parse Errors</h3>
    ${errors.map((e: any) => `<div style="font-size:11px;color:#e74c3c">${e.Path}: ${e.Error}</div>`).join('')}`;
