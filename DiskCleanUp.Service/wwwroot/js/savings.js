...
export async function updateTotalSaved() {
    try {
        ...
        if (el)
            el.textContent = data.totalBytes > 0 ? `💾 ${fmt(data.totalBytes)} freed` : '';
    }
    catch (ex) {
...
async function _fetchAndRenderSavings(append) {
    try {
...
export function filterSavings() {
    ...
    if (status)
        status.textContent = text || action ? `${visible} of ${total}` : '';
}
...
export async function restoreSavingsSelected() {
    ...
    try {
        ...
        if (status)
            status.textContent = `Restoring ${matches.length} of ${paths.length} items…`;
...
export async function newSession() {
    const label = prompt('Start a new session? All future savings will be grouped under the new session.\n\nSession name (leave blank for auto-number):', '');
    if (label === null)
        return;
    try {
...
export async function auditBinStatus() {
    const btn = document.getElementById('binAuditBtn');
    const status = document.getElementById('savingsFilterStatus');
    if (btn) {
        btn.disabled = true;
    ...
    try {
        const bin = await apiFetch('/api/recycle-bin', {}, { timeout: 35000 });
        const binPaths = new Set((bin.items || []).map((item) => (item.originalPath || '').toLowerCase()));
        ...
        document.querySelectorAll('#savingsLog tbody tr').forEach(tr => {
            ...
            if (action !== 'trash')
                return;
            trashTotal++;
            const path = tr.dataset.savingsPath || '';
        ...
        if (status && trashTotal === 0)
            status.textContent = 'No trash entries in visible rows.';
    }
    catch (e) {
        ErrLog.log('[SAVINGS]', e.message, e.stack, 'CAUGHT_ERROR');
        if (status)
            status.textContent = '⚠️ Bin audit failed';
    }
    finally {
        if (btn) {
            btn.disabled = false;
            btn.textContent = '🗑 Bin Status';
        }
    }
}
...
document.addEventListener('click', async (e) => {
    const btn = e.target.closest('.btn-verify-hardlink');
    if (!btn)
        return;
    ...
    try {
        ...
        if (res.isHardlinked) {
            const others = res.links.filter(l => l.toLowerCase() !== path.toLowerCase());
