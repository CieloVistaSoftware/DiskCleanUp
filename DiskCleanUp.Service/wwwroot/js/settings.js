...
export async function loadSettings() {
    try {
        ...
        if (_rb)
            _rb.style.display = cfg.root ? '' : 'none';
...
export async function saveSettings() {
    ...
    if (_rb2)
        _rb2.style.display = cfg.root ? '' : 'none';
    // Save trace toggle to localStorage (takes effect on next page load)
    const traceEl = document.getElementById('cfgTrace');
    if (traceEl)
        localStorage.setItem('dcu_trace', traceEl.checked ? 'true' : 'false');
}
...
function _wireRootEditable() {
    ...
    const input = document.getElementById('rootEditInput');
    if (!display || !input || display.dataset.wired)
        return;
    display.dataset.wired = '1';
    ...
    const _commit = async () => {
        ...
        if (!newRoot || newRoot === display.textContent?.trim())
            return;
        try {
            const cfg = await apiFetch('/api/config');
            cfg.root = newRoot;
            ...
            const cfgRootEl = document.getElementById('cfgRoot');
            if (cfgRootEl)
                cfgRootEl.value = newRoot;
...
export async function loadSettingsHelp() {
    ...
    try {
        ...
        if (!res.ok) {
            el.textContent = '(help file not found)';
            return;
        }
